...
iterate func list =
    case list of
        ...
        _ ->
            [ func (listHead list) ] ++ iterate func (List.map func <| listTail list)
...
calcToList calc vals angle1 angVel1 angle2 angVel2 =
    case angle1 of
        ...
        _ ->
            [ calc vals (listHead angle1) (listHead angVel1) (listHead angle2) (listHead angVel2) ] ++ calcToList calc vals (listTail angle1) (listTail angVel1) (listTail angle2) (listTail angVel2)
...
addList list1 list2 =
    case list1 of
        ...
        _ ->
            [ listHead list1 + listHead list2 ] ++ addList (listTail list1) (listTail list2)
...
rungeKutta calc_a1 calc_a2 vals angle1 angVel1 angle2 angVel2 =
    let
        ...
        k1_v1 =
            calcToList calc_a1 vals angle1 angVel1 angle2 angVel2

        k1_a2 =
            angVel2

        k1_v2 =
            calcToList calc_a2 vals angle1 angVel1 angle2 angVel2
        ...
        angVel1_ =
            addList angVel1 <| List.map (\x -> dt * x / 6) <| List.foldr addList k1_v1 [ k2_v1, k2_v1, k3_v1, k3_v1, k4_v1 ]

        angle1_ =
            addList angle1 <| List.map (\x -> dt * x / 6) <| List.foldr addList k1_a1 [ k2_a1, k2_a1, k3_a1, k3_a1, k4_a1 ]

        angVel2_ =
            addList angVel2 <| List.map (\x -> dt * x / 6) <| List.foldr addList k1_v2 [ k2_v2, k2_v2, k3_v2, k3_v2, k4_v2 ]

        angle2_ =
            addList angle2 <| List.map (\x -> dt * x / 6) <| List.foldr addList k1_a2 [ k2_a2, k2_a2, k3_a2, k3_a2, k4_a2 ]
    in
    { angVel1 = angVel1_
    , angle1 = angle1_
    , angVel2 = angVel2_
        ...
        Tick time ->
            case model.calculationMethod of
                RungeKutta ->
                    ...
                    let
                        vals =
                            { mass1 = model.mass1, mass2 = model.mass2, length1 = model.length1, length2 = model.length2 }

                        fourParams =
                            rungeKutta calc_a1 calc_a2 vals model.angle1 model.angVel1 model.angle2 model.angVel2
                    in
                    ( { model
                ...
                Euler ->
                    ...
                    let
                        ...
                        angVel1_ =
                            eulerV calc_a1 vals model.angle1 model.angVel1 model.angle2 model.angVel2 model.angVel1

                        angle1_ =
                            addList model.angle1 (List.map ((*) dt) angVel1_)

                        angVel2_ =
                            eulerV calc_a2 vals model.angle1 model.angVel1 model.angle2 model.angVel2 model.angVel2

                        angle2_ =
                            addList model.angle2 (List.map ((*) dt) angVel2_)
                    in
                    ( { model
                        | angle1 = angle1_
                        , angVel1 = angVel1_
...
createPendulums vals angle1 angle2 =
    case angle1 of
        ...
        _ ->
            List.append (dualPendulum vals (listHead angle1) (listHead angle2)) (createPendulums vals (listTail angle1) (listTail angle2))
...
dualPendulum vals angle1 angle2 =
    let
        ball1_x =
            toString <| 150 + vals.length1 * sin angle1

        ball1_y =
            toString <| 150 - vals.length1 * cos angle1

        ball2_x =
            toString <| 150 + vals.length1 * sin angle1 + vals.length2 * sin angle2

        ball2_y =
            toString <| 150 - vals.length1 * cos angle1 - vals.length2 * cos angle2

        rad1 =
            toString <| 5 * sqrt vals.mass1

        rad2 =
            toString <| 5 * sqrt vals.mass2
    in
